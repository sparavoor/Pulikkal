...
async function getCategories(sectorId?: string) {
    // If Admin viewing specific sector, show global + sector specific?
    // If Admin viewing Division (no sector), show Global only?
    // Let's just fetch all global + relevant sector categories if selected.
    const where: { OR?: { sectorId: string | null }[]; sectorId?: null } = {};
    if (sectorId && sectorId !== 'all') {
        where.OR = [{ sectorId: null }, { sectorId }];
    }
    // If 'all' (or undefined), remove strict filters to show everything
    // existing logic had 'else { where.sectorId = null }' implicitly via initialization or previous logic?
    // Let's ensure 'all' returns everything.
...
async function getCashbook(sectorId?: string, categoryId?: string) {
    const where: { sectorId?: string; categoryId?: string } = {};
    if (sectorId && sectorId !== 'all') {
        where.sectorId = sectorId;
    }
    // If sectorId is 'all' or undefined, we show ALL entries? 
    // Or do we only show Division entries (sectorId: null)?
    // Usually Admin wants to see Aggregated Sector totals or Division's own finance?
    // "Admin Sector Finance View" suggests viewing Sector data.
    // Let's assume 'all' means ALL entries (Aggregation).
...
export default async function AdminFinancePage(props: { searchParams: Promise<{ sector?: string, category?: string }> }) {
    const searchParams = await props.searchParams;
    const sectors = await getSectors();
    const selectedSector = searchParams?.sector || 'all';
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 space-y-4 sm:space-y-0">
                    ...
                    <form className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
                        ...
                        <div className="flex space-x-2">
                            ...
                            <a href="/admin/finance" className="flex items-center rounded-md bg-white px-3 py-1.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                                Clear
                            </a>
                        </div>
                    </form>
